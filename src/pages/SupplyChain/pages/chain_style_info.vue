<template>
	<div class="chain_page_content">
		...
		<div class="flex-1">
			...
			<InfoAudit v-if="web_url == 'info_audit'"/>
		</div>
	</div>
</template>
<script>
	import StyleInfo from './StyleInfoPages/style_info.vue'
	import InfoAudit from './StyleInfoPages/info_audit.vue'
	export default{
		data(){
			return{
				tab_list:[],			//顶部导航列表
		...
		created(){
			...
			let child_arr = child_list.filter(item => {
				return item.web_url == 'chain_style_info';
			});
